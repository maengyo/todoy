...
- **팔다리가 실제로 움직이는 픽셀아트 스프라이트 캐릭터** — `blocky`(게임풍
  블록형 인체), `slime`, `knight`는 프레임 단위로 그려집니다: 걷기 사이클에서
  다리가 교차하고 팔이 역스윙하며(이동 거리에 보폭 잠금 — 발 미끄러짐 없음),
  대기/점프/손흔들기 상태가 행동에 따라 전환됩니다. 전부 코드로 정의된
  오리지널 아트:
...
투명한 캐릭터가 화면에 상주하며 설정한 주기(기본 30분, `[general]
reminder_interval_minutes`)마다 오늘의 할 일을 알려줍니다 — 그리고 캐릭터마다
자기답게 움직입니다:

- **해양 생물**(고래·문어·게·펭귄)은 화면 아래 "물속"에서 푱! 하고 솟구쳐
  등장하고, 물결 위에서 둥실거리다, 리마인드가 울리면 잠수했다 다시 솟아오릅니다.
- **비행 생물**(나비·벌·부엉이·오리·용)은 화면 **위쪽**을 날아다니며 메시지를
  다리에 매단 플랜카드로 끌고 다니고, 알림이 울리면 스웁을 합니다.
- 토끼·개구리는 통통 튀며 입장, 유령·외계인·로봇은 깜빡이며 물질화, 나머지는
  화면 밖에서 걸어 들어옵니다.
- 이동 기본값은 **auto** — 캐릭터가 자기 본성에 맞는 프리셋을 고릅니다
  (말은 질주, 나비는 부유, 벌은 돌진…). `movement`를 명시하면 그게 우선.
