...
- **애니메이션 선택 (2026-08-20 추가).** 캐릭터 이동(`walk`/`hop`/`float`/`dash`/`still`)과 말풍선 등장(`pop`/`fade`/`slide`/`shake`/`none`)을 config(`[display] movement / bubble_effect`) 또는 `todoy overlay --movement/--bubble-effect`로 선택할 수 있다.
