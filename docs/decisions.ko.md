...
- **애니메이션 스프라이트 캐릭터 (이슈 #18).** 사용자 피드백: 이모지 이동은 애니메이션이 아니다. 오리지널 픽셀아트 팩(blocky/slime/knight — 게임풍이지만 자체 제작; 저작권 있는 게임 캐릭터는 번들 불가)을 전부 코드로 정의(`display/pixelart.py`의 픽셀 그리드+팔레트, 바이너리 에셋 0). 각자 대기/걷기/점프/손흔들기 상태; 걷기는 진짜 관절 움직임(다리 교차, 팔 역스윙, 1px 바운스).
- **렌더러.** 픽셀 그리드 → nearest-neighbor 캐시 NSImage(~6배, 선명). 순수 상태머신(`display/overlay/spritestate.py`, 전 OS CI 테스트)이 행동으로 상태 결정: 등장점프 > 플러리시 손흔들기 > 공중 점프 > 걷기(프레임 인덱스를 이동 거리에 잠금 — 발 미끄러짐 없음) > 대기(~2fps, 이징 턴 중 강제). 방향 미러/스쿼시/등장 알파는 그대로; 이모지 캐릭터 영향 없음.
- **내 스프라이트 가져오기.** `[display] character_sprites`에 `<state>_<n>.png` 폴더 지정(폴백: jump→walk[0], wave→idle; ≤96px) — 소장한 어떤 스프라이트든 애니메이션 가능.

### 교차 리뷰 반영 수정

- 순수 스프라이트 로직을 macos.py에서 분리해 Linux/Windows CI에서도 테스트; 턴 판정을 per-tick dx 대신 `movement.is_turning`으로; knight ASCII 폴백 프레임 수리; 병렬 개발 잔재 제거.

## M13 (후속) — 2026-08-22

### 결정

- **캐릭터 페르소나 (이슈 #17).** 모든 캐릭터가 Persona(서식지/등장/플러리시/배너/기본 이동)를 가짐 — `display/overlay/personas.py`: 해양 생물은 화면 아래에서 푱 솟구쳐 등장하고 리마인드 시 잠수-재부상; 비행 생물은 화면 상단(실제 메뉴바 높이에서 도출한 여백)을 순항하며 메시지를 두 줄로 매단 플랜카드로 운반; 점프형은 튀며 입장; 유령형은 깜빡이며 물질화; 나머지는 걸어 입장. 등장/플러리시 커브는 순수 dt 구동 상태머신.
- **movement="auto"가 새 기본값**: CLI에서 OverlayOptions 생성 전에 페르소나의 본성 프리셋으로 해석(하류로 절대 누출 안 됨); 명시값 우선; 기존 명시 설정 불변.
- **사용자 제보 수정:** 물 캐릭터에서 깃발이 실제로 지연/이탈했음 — pop/slide 등장 효과가 Core Animation으로 창 프레임을 움직여 30fps 동행 틱과 충돌(flag 스타일은 알파 전용 등장으로 변경); 말풍선 패널 ~11% 축소 + 버튼 소형화.

### 교차 리뷰 반영 수정

- 등장 중 울린 리마인드의 플러리시는 드롭 대신 지연 실행; 상단 여백을 하드코딩 24px 대신 NSScreen frame-vs-visibleFrame에서 도출; 배너 줄을 두 발에서 분산; 깃발 추적 회귀 보강(반복 발화, 가장자리 클램프 출발).

## M12 (후속) — 2026-08-22

### 결정

- **부드러운 움직임 (이슈 #16).** 오버레이 배회 틱을 30fps로 상향(M11에서 이동을 dt 독립으로 만들어둔 덕에 평균 속도 불변). 가장자리 턴에 이징 도입: ~0.3초 감속 → 속도 0 지점에서 방향 반전 → 가속, C1 연속(속도 점프 없음); 갤럽 스쿼시도 부드럽게 보간. 실측: 턴 구간 속도 궤적 단조 감속·가속, CPU ~3-4%.
- **카탈로그 25종.** + fox, panda, chick, rabbit, hamster, duck, whale, octopus, butterfly, dragon. 새 규칙: 모든 카탈로그 캐릭터가 ≥2 ASCII 걸음 프레임 보유(rabbit은 3프레임 홉) + 캐릭터별 보폭 — 터미널 러너가 전부 애니메이션; 정지 아트 폴백은 카탈로그 이름에 대해 도달 불가.

## M11 (후속) — 2026-08-21

### 결정

- **`todoy run` — 크로스 플랫폼 터미널 라이브 모드 (이슈 #15).** 순수 `\r` 재그리기 단일 라인 마퀴(ANSI·의존성 0 — Linux/Windows/macOS 터미널 동일 동작): 캐릭터가 왼쪽→오른쪽으로 달리고 티커처럼 순환(뒤로 달리는 일 없음), 한 줄 깃발 메시지가 2컬럼 뒤를 따름; 주기 리마인드와 시간 알람은 위에 블록으로 출력 후 질주 재개.
- **진짜 걸음걸이.** `display/sprites.py`에 캐릭터별 ASCII 걸음 사이클 — 말은 4프레임 갤럽(`,,=(oo)=,,` 다리 모음→뻗음→차기→착지) — 과 `STRIDE_COLUMNS`: 이동이 보폭에 잠김(다리 한 사이클당 정확히 N컬럼, Bresenham 분배) — 발이 미끄러지지 않음.
- **오버레이 방향·보폭 동기.** 캐릭터 뷰가 이동 방향을 보도록 미러링(애플 🐎 글리프는 왼쪽을 봄 — 오른쪽 이동이 뒤로 달리는 것처럼 보였음); 갤럽은 수평 이동의 ~80%를 공중 구간에 집중(사이클당 거리 보존) + 갤럽 전용 미세 스쿼시.

### 교차 리뷰 반영 수정

- `render_run_line`이 깃발 텍스트를 새니타이즈(ESC 시퀀스가 터미널 출력으로 샐 수 있었음).
- 갤럽이 phase 경계를 넘는 dt를 구간별로 적분 — 경계에 안 맞는 dt에서 사이클당 거리가 틀어지던 문제 수정(비정렬 dt 회귀 테스트).
- CLI 포맷 blocker 수정; 실제 서브프로세스 SIGINT 스모크 테스트 커밋(POSIX 한정).

## M10 (후속) — 2026-08-20

### 결정

- **컴팩트 펄럭이는 깃발.** 사용자 피드백: 깃발 패널이 너무 큼. `message_style="flag"`는 이제 핵심 **한 줄**만 담은 페넌트(~34px, 버튼 없음) — `build_flag_line`("할 일 N개: 첫 항목 (+k)", 표시 폭 38컬럼 제한) 또는 `build_alarm_flag_line`("⏰ HH:MM …") — 이 캐릭터에 붙어 달리며 펄럭이고(6Hz 노치 웨이브), 10초 후 자동 숨김, 클릭하면 스누즈(알람 인지). `bubble`은 기존 상세형 유지(30초, Snooze/Quit 버튼).
- 종료는 어느 스타일에서도 가능: 메뉴바 상태 아이콘 우클릭에 "Quit todoy" 메뉴 추가.
- 캐릭터 카탈로그 15종으로 확대(+turtle, snail, penguin, frog, bee, owl, unicorn, dino, alien, crab); 영어 taunt의 단복수 오류("1 things") 수정.
- README 데모 미디어는 라이브 앱에서 자가 렌더링 (docs/demo.ko.md 참고).

### 교차 리뷰 반영 수정

- FLAG_MAX_WIDTH 280→500px — 합법적인 최대 38컬럼 줄이 ~464px로 잘릴 수 있었음; 모든 최대 폭 빌더 출력을 상한과 대조하는 회귀 테스트 추가.
- Quit 메뉴 테스트를 bubble 스타일까지 확장.

## M9 (후속) — 2026-08-20

### 결정

- **메뉴바 퀵애드 패널 (이슈 #14).** 오버레이 실행 중 NSStatusItem이 패널을 엶: 퀵애드(텍스트 + 선택적 HH:MM, 리턴 제출, 인라인 새니타이즈 에러) + 오늘의 리스트(builtin 할 일에 ✓ 완료 / ✕ 삭제 / 📌 고정 컨트롤; 노트에서 온 할 일은 읽기 전용 — todoy는 사용자 노트를 절대 수정하지 않음). 리마인드 말풍선은 스누즈/종료만 유지 — 관리 행위는 의도적으로 분리.
- **모델.** `Todo.pinned` + `Todo.created`(YYYY-MM-DD, 하위 호환); `BuiltinSource.set_pinned/delete/sweep`.
- **일일 청소.** `[general] daily_clear = false` 기본(옵트인 — 조용한 데이터 삭제는 선택이어야 함). 켜면 builtin을 만지는 모든 명령이 오늘 이전에 생성된 고정 안 된 할 일(완료 여부 무관)을 청소; created가 없는 행은 오늘 날짜로 스탬프(마이그레이션)하고 삭제하지 않음.
- **PanelActions.** 백엔드는 예외를 던지지 않는 콜러블들(에러 문자열 또는 None)의 frozen dataclass를 받음 — GUI 코드와 저장소의 결합 차단.
- CLI: `todoy pin/unpin/rm <id>`, `add --pin`; 고정 항목은 뒤에 📌 표시.

## M8 (후속) — 2026-08-20

### 결정

- **시간 알람 (이슈 #13).** `Todo.at`(`"HH:MM"`, `parse_at`으로 제로패딩 정규화) — `todoy add --at` 또는 마크다운 줄 선두 시간 토큰(`- [ ] 14:00 회의`)에서. 오버레이의 `AlarmClock`(순수, 주입형 시계)이 시간 지정 할 일을 하루 한 번 해당 분에 발화 — **그 메시지만** 표시(⏰ 줄, taunt 없음) — 놓친 분은 10분 한도 내에서 한 번 따라잡고, 스누즈 시 재예약. 주기 리마인드와 독립. 시간 지정 할 일은 list/TUI/오버레이에서 `HH:MM ` 접두사로 렌더링.
- `OverlayBackend.run`에 `get_todos` 콜러블 추가 — 백엔드가 매 틱 신선한 할 일을 읽음.

### UI 폴리시 (같은 날)

- 새 ui-polish-expert 에이전트가 오버레이 말풍선/깃발 리스타일: 타이포 위계, 액센트 Snooze 필 + 조용한 Quit, 그린 말풍선 꼬리와 깃발 페넌트+폴, 시스템 컬러(다크/라이트), 부드러운 radius/그림자/패딩.
- 실행 검증 중 CLI 버그 2건 발견·수정: argparse 선택지를 animations 상수에서 가져오도록(gallop 누락됐었음), `todoy overlay --character` 플래그 추가.
